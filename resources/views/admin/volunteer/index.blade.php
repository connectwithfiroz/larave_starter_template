                                ...
                                <table id="donation_list_table"
                                    ...
                                    <tbody>
                                        @forelse ($volunteers as $volunteer)
                                            <tr>
                                                <td>{{ $volunteer->id }}</td>
                                                <td>{{ $volunteer->regisration_id }}</td>
                                                <td>{{ $volunteer->full_name }}</td>
                                                <td>{{ $volunteer->email }}</td>
                                                <td>{{ $volunteer->phone }}</td>
                                                <td>₹{{ number_format($volunteer->amount, 2) }}</td>
                                                <td>
                                                    @if (is_null($volunteer->razorpay_payment_id))
                                                        <span class="badge bg-warning text-dark">Pending</span>
                                                    @elseif ($volunteer->razorpay_payment_id && is_null($volunteer->razorpay_signature))
                                                        <span class="badge bg-danger">Failed</span>
                                                    @else
                                                        <span class="badge bg-success">Success</span>
                                                    @endif
                                                </td>
                                                <td>{{ $volunteer->razorpay_payment_id ?? '-' }}</td>
                                                <td>{{ $volunteer->razorpay_order_id ?? '-' }}</td>
                                                <td>{{ $volunteer->created_at->format('d M Y, h:i A') }}</td>
                                            </tr>
                                        @empty
                                            <tr>
